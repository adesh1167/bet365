...
export default function UploadTickets({init}){
	...
	function generateSettled(){
		...
		for(let i=0; i<games.length; i++){
            ...
            let betId = lines[0].split(':')[1].trim();
            let stakeTime = lines[3].replace('| ', '');
            lines = lines.filter((_, i)=>(i>4));
	...
	function upload(data, btn){
		if(data.length > 10 && JSON.parse(data)){
			...
			window.$.ajax({
				url: `${baseApiUrl}/handlers/update-tickets.php`,
				type: 'POST',
				data: {tickets: JSON.parse(data), user: window.allParams.user},
				dataType: 'JSON',
				success: (res)=>{
	...
	return (
		<div className="ut-home">
			...
			<div className="close-ut fa fa-close" onClick={()=>{setPopup('fullMenu')}}>&times;</div>
		</div>		
	)	
